---
title: "Helena Gregg | Software Engineer & MBA"
description: "Professional portfolio of Helena Gregg, Software Engineer at Two Sigma/Microsoft and MBA candidate at Rice University specializing in Treasury Engineering."
keywords: ["Helena Gregg", "Software Engineer", "Two Sigma", "Microsoft", "Rice MBA", "C++", "Treasury Engineering"]
---

**Software Engineer | MBA Candidate at Rice University**

[LinkedIn Profile](https://www.linkedin.com/in/helena-gregg/) | [GitHub](https://github.com/HelenaGregg)

{{< slideshow speed=5000 items="/gallery/north-campus.jpg|University of Michigan North Campus,/gallery/2-sigma.jpg|Two Sigma,/gallery/rice.jpeg|Rice MBA,/gallery/lincoln-park.jpg|Lincoln Park Honeycomb,/gallery/lakefront-trail.jpg|Lakefront Trail,/gallery/snow-leopard.jpg|Snow Leopard in Lincoln Park Zoo" >}}

I currently spend my days on the **Treasury Engineering** team at Two Sigma, while spending my **evenings and weekends** completing my MBA in Finance at Rice University.

* **Next Chapter:** Trading the Houston heat for the Lake Michigan breeze—moving to **Chicago this May** following the conclusion of my grad program!
* **In the Garden:** Currently obsessed with heirloom varieties and figuring out the "Zone 5b" Chicago growing season.
* **On the Pavement:** Marathon runner and future Lakefront Trail enthusiast.
* **In the Wild:** Usually hiking or finding any excuse to get away from a screen.

Always happy to connect with fellow Owls, Chicago-based engineers, or anyone with tips on keeping tomatoes alive in the Midwest!

---

## Experience

### **Two Sigma** | Software Engineer (Treasury Engineering)
*September 2021 – Present*
- Engineered high-performance systems within the Trading Engineering organization.
- Focused on Treasury Engineering, optimizing capital allocation and financial workflows.

### **Microsoft** | Software Engineer (Visual Studio C++ Team)
*June 2020 – August 2021*
- Worked on the **C++ Compiler Backend Team**.
- Contributed to compiler optimizations and code generation for one of the world's most used development toolchains.

### **Microsoft** | Software Engineering Intern
*Summer 2019*
- Developed features for the Visual Studio C++ Compiler Backend.
- Gained deep exposure to systems programming and enterprise-scale CI/CD.

### **University of Michigan** | Instructional Assistant
*2018 – 2020*
- Taught "Introduction to Computer Organization."
- Led lab sections, graded projects, and mentored students in low-level hardware/software interfacing.

---

## Education

### **Rice Business - Jones Graduate School of Business**
*Master of Business Administration (MBA), Finance* **2024 – 2026 (Expected)**

### **University of Michigan College of Engineering**
*Bachelor of Science in Engineering (B.S.E.), Computer Science* **2017 – 2020**
* **Honors:** Summa Cum Laude (GPA: 3.9/4.0)

---

## Skills

- **Languages:** C++, C, Python, Java, SQL, PowerShell.
- **Systems:** Compiler Design, Treasury Management Systems, Computer Architecture.
- **Tools:** Git, Visual Studio, Hugo, GitHub Actions, Linux/Unix.